...
class CommandHandler {
    ...
    constructor(instance: DJSCommands, client: Client, dir: string, typeScript = false) {
        this.client = client;
     ...
     * @param instance - DJSCommands instance
     * @param client - Discord client
     * @param dir - commands directory
     * @param typeScript - if program runs as TypeScript
     * @private
     */
    private async checkAndSetup(instance: DJSCommands, client: Client, dir: string, typeScript: boolean): Promise<void> {
        if (dir) {
            if (!existsSync(dir))
            ...
            console.log(`DJSCommands > Loaded ${amount} command${amount === 1 ? "" : "s"}.`);
     ...
     * @private
     */
    private async registerCommand(instance: DJSCommands, client: Client, file: string, fileName: string): Promise<void> {
        ...
        if (missing.length && instance.showWarns)
            console.warn(`DJSCommands > Command "${names[0]}" does not have the following properties: ${missing}.`);

        if (testOnly && !instance.testServers?.length)
            console.warn(`DJSCommands > Command "${names[0]}" has "testOnly" set to true, but no test servers are defined.`);

        if (typeof slash !== "boolean")
            throw new Error(`DJSCommands > Command "${names[0]}" has a "slash" property that is not boolean "true".`);
        if (!slash && options.length)
            throw new Error(`DJSCommands > Command "${names[0]}" has an "options" property but is not a slash command.`);
        if (slash) {
            if (!description)
                throw new Error(`DJSCommands > A description is required for command "${names[0]}" because it is a slash command.`);
            ...
            if (options.length) {
                for (const key in options) {
                    ...
                    if (name !== lowerCase && instance.showWarns)
                        console.log(`DJSCommands > Command "${names[0]}" has an option of "${name}". All option names must be lower case for slash commands. DJSCommands will modify this for you.`);

                    if (lowerCase.match(/\s/g)) {
                        lowerCase = lowerCase.replace(/\s/g, "_");
                        console.log(`DJSCommands > Command "${names[0]}" has an option of "${name}" with a white space in it. It is a best practice for option names to only be one word. DJSCommands will modify this for you.`);
                    }
